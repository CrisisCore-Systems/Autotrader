...
from .backtest import BounceHunterBacktester
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--earnings-window", type=int, default=5, help="Blackout window in calendar days around earnings")
    parser.add_argument(
        "--allow-earnings",
        action="store_true",
        help="Allow signals inside the earnings window (disabled by default)",
    )
    parser.add_argument(
        "--backtest",
        action="store_true",
        help="Run a walk-forward backtest instead of scanning for today's signals",
    )
    parser.add_argument(
        "--backtest-start",
        type=str,
        help="Optional start date (YYYY-MM-DD) for the backtest evaluation window",
    )
    parser.add_argument(
        "--backtest-end",
        type=str,
        help="Optional end date (YYYY-MM-DD) for the backtest evaluation window",
    )
    parser.add_argument(
        "--training-events",
        type=int,
        default=0,
        help="Limit the backtest models to the most recent N labelled events (0 uses full history)",
    )
    parser.add_argument(
        "--show-trades",
        action="store_true",
        help="Print the individual trades generated by the backtest",
    )
...
def configure(args: argparse.Namespace) -> BounceHunterConfig:
    config = BounceHunterConfig(
        ...
        earnings_window_days=max(args.earnings_window, 0),
    )
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    if args.backtest:
        backtester = BounceHunterBacktester(
            ...
            max_training_events=max(args.training_events, 0),
        )
