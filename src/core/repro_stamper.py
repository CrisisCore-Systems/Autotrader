...
class ReproStamper:
    ...
    def _hash_codebase(self) -> str:
        ...
        try:
            ...
            files = [self.workspace_root / f.strip() for f in result.stdout.split("\n") if f.strip()]
    ...
    def _validate_git_commit(self, stamp: ReproStamp, strict: bool) -> List[str]:
        """Validate git commit against stamp."""
        errors = []
        if not stamp.git_commit:
            return errors

        git_info = self._get_git_info_cached()

        if git_info["commit"] != stamp.git_commit:
            errors.append(f"Git commit mismatch: expected {stamp.git_commit}, got {git_info['commit']}")
    ...
    def _validate_input_hashes(self, stamp: ReproStamp, input_files: Optional[List[Path]]) -> List[str]:
        """Validate input file hashes against stamp."""
        errors = []
        if not (input_files and stamp.input_hashes):
            return errors
        ...
        for file_path, expected_hash in stamp.input_hashes.items():
            ...
            if current_hash != expected_hash:
                errors.append(
                    f"Input file hash mismatch: {file_path} " f"(expected {expected_hash}, got {current_hash})"
                )
        return errors

    def _validate_config_hash(self, stamp: ReproStamp, config_data: Optional[Dict[str, Any]]) -> List[str]:
        """Validate config hash against stamp."""
        errors = []
        if not (config_data and stamp.config_hash):
            return errors

        current_hash = self._hash_dict(config_data)
        if current_hash != stamp.config_hash:
            errors.append(f"Config hash mismatch: expected {stamp.config_hash}, got {current_hash}")
        return errors

    def _log_validation_result(self, valid: bool, errors: List[str]) -> None:
    ...
    def validate_stamp(
        ...
        """
        errors = []
        errors.extend(self._validate_git_commit(stamp, strict))
        errors.extend(self._validate_input_hashes(stamp, input_files))
        errors.extend(self._validate_config_hash(stamp, config_data))

        valid = len(errors) == 0
        self._log_validation_result(valid, errors)
